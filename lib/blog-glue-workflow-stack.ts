...
export class BlogGlueWorkFlowStack extends cdk.Stack {
  ...
  constructor(scope: cdk.App, id: string, props?: cdk.StackProps) {
    ...
    const f_pyAssetETL = new Asset(this, "test-helloworld", {
      path: path.join(__dirname, "assets/helloworld.py"),
    });
    const f_pyParquet = new Asset(this, "test-helloatom", {
      path: path.join(__dirname, "assets/helloatom.py"),
    });
    const f_pyRedshiftLoad = new Asset(this, "test-hellojaden", {
      path: path.join(__dirname, "assets/hellojaden.py"),
    });
    ...
    const glue_db = new glue.Database(this, "atom-test-cdk-create-glue-database", {
      databaseName: "atom-test-cdk-create-glue-database",
    });

    //create glue cralwer role to access S3 bucket
    // const glue_crawler_role = new Role(this, "atom-cdk-test-create-role", {
    //   roleName: "atom-cdk-test-create-role",
    //   description:
    //     "atom-cdk-test-create-glue-role by Jaden",
    //   managedPolicies: [
    ...
    const glue_workflow = new glue.CfnWorkflow(this, "atom-test-cdk-create-workflow", {
      name: "atom-test-cdk-create-workflow",
      description:
        "just a test demo for cdk create, hello world, hello atom and hello jaden!",
    });

    //create jobs
    const glue_job_asset = new glue.CfnJob(this, "atom-test-cdk-create-job-helloworld", {
    ...
    const glue_job_parquet = new glue.CfnJob(this, "atom-test-cdk-create-job-helloatom", {
